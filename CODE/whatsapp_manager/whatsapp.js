  ...
  async connect() {
    try {
      ...
      this.sock.ev.on('connection.update', async (update) => {
        ...
        // Armazena informações sobre a última desconexão
        if (lastDisconnect) {
          this.state.lastDisconnect = lastDisconnect;
          
          // Tenta reconectar em caso de erro de desconexão não fatal
          const statusCode = lastDisconnect.error?.output?.statusCode;
          if (statusCode && statusCode !== DisconnectReason.loggedOut) {
            this.logger.info('Desconexão não fatal, tentando reconectar...');
            await this.connect();
          } else if (statusCode === DisconnectReason.loggedOut) {
            this.logger.info('Desconectado (logout)');
            this.emit('logout');
          }
        }
