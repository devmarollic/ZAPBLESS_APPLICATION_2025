...
import { EventController } from './event_controller.js';
...
export class WebhookController extends EventController
{
    ...
    {
        super( supabaseRepository, whatsappMonitor, true, 'webhook' );
    }
        ...
        )
    {
        ...
        {
            if ( data.webhook.events.length === 0 ) 
            {
                data.webhook.events = EventController.events;
            }
        }
    
        return this.supabaseRepository.upsert(
            'WEBHOOK',
            {
                where:
                    {
                        instanceId: this.whatsappMonitor.waInstances[ instanceName ].instanceId
                    },
                update:
                    {
    ...
    )
    {
        ...
        const webhookConfig = process.env.WEBHOOK;
        const webhookLocal = instance?.events;
        const webhookHeaders = { ...( instance?.headers || {} ) };
    
        if ( webhookHeaders
             && 'jwt_key' in webhookHeaders )
        {
        ...
        const enabledLog = process.env.LOG.LEVEL.includes('WEBHOOKS');
        const regex = /^(https?:\/\/)/;
        ...
        const webhookData =
            {
                ...
                data,
                destination: instance?.url || `${ process.env.WEBHOOK.GLOBAL.URL }/${ transformedWe }`,
                date_time: dateTime,
                sender,
                        ...
                        const httpService = axios.create(
                            {
                                ...
                                timeout: process.env.WEBHOOK.REQUEST?.TIMEOUT_MS ?? 30000
                            }
                            );
        ...
        {
          if ( process.env.WEBHOOK.EVENTS[ we ] )
          {
            let globalURL = process.env.WEBHOOK.GLOBAL.URL;
    
            if ( process.env.WEBHOOK.GLOBAL.WEBHOOK_BY_EVENTS )
            {
    ...
    {
        const webhookConfig = process.env.WEBHOOK;
        const maxRetryAttempts = maxRetries ?? process.env.WEBHOOK.RETRY?.MAX_ATTEMPTS ?? 10;
        const initialDelay = delaySeconds ?? process.env.WEBHOOK.RETRY?.INITIAL_DELAY_SECONDS ?? 5;
        const useExponentialBackoff = process.env.WEBHOOK.RETRY?.USE_EXPONENTIAL_BACKOFF ?? true;
